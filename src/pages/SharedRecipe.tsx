...
interface SharedRecipeRpcResult {
  id: string;
  title: string;
...
type DbRecipeWithIngredients = Recipe & {
    ...
    const fetchSharedRecipe = async () => {
      ...
      try {
        ...
        if (!rpcError && rpcData && typeof rpcData === "object" && "title" in rpcData) {
          const raw = rpcData as unknown as SharedRecipeRpcResult;
          const formatted: RecipeWithDetails = {
            id: raw.id,
            user_id: "",
            title: raw.title,
            ...
            updated_at: new Date().toISOString(),
            ingredients: (raw.ingredients || []).map((ing, idx: number) => ({
        ...
        if (dbError || !dbRecipe) {
          if (dbError) {
            console.error("Failed to load shared recipe:", dbError);
          }
          setError("Cette recette n'est pas disponible ou le lien est invalide.");
          return;
        }

        const typedDbRecipe = dbRecipe as unknown as DbRecipeWithIngredients;
        const formatted: RecipeWithDetails = {
          ...typedDbRecipe,
          ingredients: (typedDbRecipe.ingredients || []).map((ing) => ({
            id: ing.id,
            quantity: ing.quantity,
            unit: ing.unit,
            name: ing.name,
  ...
  return (
    <>
      ...
      <div className="print:hidden min-h-screen pb-20 bg-background text-foreground">
        ...
        <div className="relative h-64 sm:h-80 md:h-96 w-full bg-muted md:rounded-2xl md:mt-4 overflow-hidden">
          <RecipeImage
        ...
        {/* Content Layout Grid */}
        <div className="lg:grid lg:grid-cols-12 lg:gap-8 items-start mt-6">
